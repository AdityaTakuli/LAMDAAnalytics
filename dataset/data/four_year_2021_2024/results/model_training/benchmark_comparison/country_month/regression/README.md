# Run artifacts — regression

Generated 2026-09-01T11:49:05.735368+00:00 on `localhost.localdomain` using device
`cpu` in 102.965s.

| Path | Contents |
| --- | --- |
| `run_summary.json` | Everything about this run: options, environment, split, per-model metrics. Start here. |
| `metrics/comparison.csv` | One row per (model, split). The headline column is `rmse`. |
