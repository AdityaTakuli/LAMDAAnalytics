...
Task: `regression` on target column `contraction`.
...
* Valid targets: 643
* Invalid targets: 317
  (future value unobserved: 20,
  baseline unavailable: 220,
  baseline not positive: 80)
* Invalid rows are excluded from training and evaluation. They are never
  ...
  or rebalanced.
* All 20 country nodes stay in every graph snapshot so the topology
